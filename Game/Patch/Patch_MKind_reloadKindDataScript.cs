...
{
    /// <summary>
    /// 注入自定义 <c>MKind</c> 的时机：挂 Postfix 而不是自己找时机调用，此时原字典和小图标序列都
    /// 已建好，构造 <c>MKind</c> 不会碰到半初始化的图标资源。<c>reloadKindDataScript</c> 自带
    /// "已加载就直接返回"的早退，这个 Postfix 可能被重复触发，注入本身是幂等的。
    /// </summary>
