...
namespace Polaris.Magic.Game
{
    ...
    /// 名字必须双向解析。
    ///
    /// <c>FEnum&lt;MGKIND&gt;.PreDefineToStr</c> 只解决"值 → 名字"，魔法菜单建按钮用它；但点击、悬停、
    /// 事件命令（ENABLEMAGIC / SETMAGIC / REMOVEMAGIC / GETMAGIC）和菜单 MAGIC_LEARN 读回时走的是
    /// <c>TryParse</c>，走的是另一张私有缓存。只登记一半的表现是"菜单里画得出来、点下去没反应"。
    ///
    /// 反向那半张表是 <c>FEnum&lt;T&gt;</c> 的私有静态字段 <c>Ostr</c>，靠 Krafs.Publicizer 直接写入：
    /// 它本身就是 <c>TryParse</c> 的缓存（原版会把成功解析过的名字写回去），预先塞一条与原版行为一致，
    /// 比再 patch 一次 <c>TryParse</c> 干净。
    /// </summary>
